...
* **Vixen Hub:** Your central control matrix. A 5-tab developer dashboard driven by a native Markdown-to-UIToolkit parser. It dynamically fetches your local VPM documentation (`README.md`, `CHANGELOG.md`, `SUPPORT.md`), enforces strict flex-box container wrapping, and utilizes an advanced regex engine to intercept Markdown links and spawn them as interactive, borderless UI Toolkit buttons. It also serves as the master launchpad for the entire suite, utilizing generic `System.Action` delegates to seamlessly route both web traffic and Unity Editor menu commands.
