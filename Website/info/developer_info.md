...
- **`VixenWearMaterials`** (static): the relocated `SyncKeywords` (VRSL/LTCGI/LV/detail keywords from float thresholds, alpha keywords from `_Mode`, clears `EmissiveIsBlack`) and `SetupMaterialWithBlendMode` (blend, `_ZWrite`, render queue, `RenderType` + `VRCFallback` tags). Shared by the editor and the build/play infrastructure.
...
Both twins. Locking a material broke the procedural effect passes (base: drip / hud / shard / trail; SPS: drip / goo) - they are full `#pragma vertex/geometry/fragment` passes (`LightMode=ForwardBase`, `#pragma require geometry`), distinct from the two `#pragma surface` passes. Thry's `ShaderOptimizer` **comments out every `#pragma geometry` line** (and every raw `#pragma hull` / `#pragma domain`) of the locked output by default, because `UseGeometry` / `UseTessellation` start `false` (`ShaderOptimizer.cs` ~L137-146, stripping ~L1959-2012). With the geometry stage gone, each effect pass collapses to a vertex->fragment pass whose vertex output no longer feeds the geometry amplification the fragment expects - the passes render wrong or fail to compile.

- **Fix.** Added `[HideInInspector] GeometryShader_Enabled ("", Float) = 1` directly under the lock button in each `Properties` block. This is Thry's documented "mega-shader" opt-out convention property: at value 1 it sets `UseGeometry = true`, so the optimizer keeps `#pragma geometry`. The per-lightmode keep flags (`UseGeometryForwardBase`, etc.) default `true`, and the effect passes are all `ForwardBase`, so all geometry pragmas survive locking (a pass with an unrecognized/no LightMode also survives, since the keep-switch has no matching case). `GeometryShader_Enabled` (without a per-pass suffix) is the global toggle; suffixed variants (`GeometryShader_EnabledForwardBase`/`ForwardAdd`/`ShadowCaster`/`Meta`) are not used.
- **Tessellation is unaffected, so no `Tessellation_Enabled` needed.** The main pass tessellates via `tessellate:tessEdge` on the `#pragma surface` line; Thry only strips raw `#pragma hull` / `#pragma domain` lines, which neither twin has. Surface-shader tessellation (and the `vertex:disp` / `vertex:outlineDisp` surface vertex functions) are part of `#pragma surface`, which the optimizer leaves intact - consistent with the VixenWorld tessellated-surface lock note above.
- **No cost delta.** One hidden constant float per twin; no samplers, keywords, variants, or runtime cost. Re-lock the material in Unity after this change and confirm the locked (`Hidden/Locked/...`) output retains its `#pragma geometry` lines and the effect passes render.

### `only_renderers` lists d3d11 + glcore + vulkan (PC-future readiness, inert under VRChat today) (2026-06-29)

Both twins. All ten `#pragma only_renderers` lines (base: 2 surface + 4 geometry passes; SPS: 2 surface + 2 geometry passes) read `#pragma only_renderers d3d11 glcore vulkan` instead of the old `d3d11`. `d3d11` already covers Direct3D 12 (same DXBC target; there is no separate `d3d12` token), `glcore` is desktop OpenGL Core, `vulkan` is Vulkan - all three support the SM5 surface tessellation (`#pragma target 5.0` + `tessellate:tessEdge`) and the geometry stage the effect passes need. Reference for the multi-renderer pattern: VRChat's own `com.vrchat.base/.../PanoViewer/Panosphere.shader` (`#pragma only_renderers d3d9 d3d11 glcore gles …`); `ToonStandard` sets no restriction at all.

**This is forward-readiness only; it is inert in VRChat's current pipeline - do not bill it as a live "now works on OpenGL/Vulkan" fix.** The VRChat SDK's `EnvConfig` (`com.vrchat.base/Editor/VRCSDK/Dependencies/VRChat/EnvConfig.cs`, `allowedGraphicsAPIs` L46-54, applied by `SetGraphicsAPIs` L513-545) force-overwrites Player Settings -> Graphics APIs on every editor load: StandaloneWindows64 -> `Direct3D11` only, Android -> `OpenGLES3` (`/* Vulkan */` still commented out), iOS/OSX -> `Metal`. So a VRChat PC upload compiles **only** D3D11 variants regardless of `only_renderers`, and the `glcore`/`vulkan` tokens produce no compiled variant today. They are kept (decision 2026-06-29) so that if/when VRChat enables desktop Vulkan/DX12 for PC, these passes are already declared for it with no shader edit needed.

**Quest/Android is NOT a target and cannot be made one at the shader level.** Beyond GLES3 having no geometry stage and no SM5 tessellation, VRChat replaces any non-whitelisted shader on Android for **both** avatars and worlds: `AvatarValidation.ShaderWhiteList` (`com.vrchat.base/.../Validation/AvatarValidation.cs` L109-123) and `WorldValidation.ShaderWhiteList` (L531-546) allow only `VRChat/Mobile/*` (plus `FX/MirrorReflection`, `UI/Default` for worlds). `VixenWear/Latex Ultra` is custom, so it is swapped out on Quest - no GLES3/Vulkan gating in this file would ever execute there. Hence `gles`/`gles3`/`metal` are intentionally omitted.

- **Geometry passes keep `#pragma require geometry`**, so any backend without a geometry stage skips them gracefully rather than failing to compile.
- **No cost delta under VRChat** (only D3D11 is built). If this material is ever used OUTSIDE VRChat (a standalone Unity project with Vulkan/glcore in Player Settings), the surface-pass variant count triples and the glcore/vulkan variants need a Unity test-compile - the large HLSL surface + four geometry passes (per-texture `GetDimensions`, single-pass-stereo geometry amplification) may surface HLSL->GLSL/SPIR-V translation issues (VR single-pass-stereo geometry eye-index is the likeliest spot) that only appear when those variants are actually built.
...
- **L4** — integrations (AudioLink, LTCGI, VRSL + VRSL GI, VRC Light Volumes)  <br/><sub>↳ before `Shader "VixenWear/Latex Ultra"`</sub>
...
### Lighting refresh + wet/parallax/matcap fixes, AreaLit removal (2026-06-25)

- **Parallax self-shadow fix (`ParallaxRaymarchShadow`, `surf`).** Enabling Parallax Depth could black out the whole surface. Two bugs: (1) `o.Height` was written as `pbrHeight * _Disp_Str`, but `s.Height` is consumed ONLY as the shadow ray's start height, which the loop compares against raw `[0,1]` height-map samples - with `_Disp_Str` low/0 the ray started below the field and the first `diff < 0` returned `0.0` (fully shadowed). Now `o.Height = pbrHeight` (raw). (2) The loop tracked `minShadow = min(minShadow, diff * softness)` where `diff = rayHeight - mapHeight` (the lit margin), so on a flat surface the first tiny margin locked it to ~3% (near-black). Rewrote it to accumulate occlusion: `occ = mapHeight - rayHeight; if (occ > 0) shadow = max(shadow, occ * strength); return saturate(1 - shadow)`. Flat surfaces return `1.0`; `strength = lerp(1, 8, saturate(hardness))` maps `_Shad_Hard` to contact-shadow contrast. Constants are first-pass; tune in Unity.
- **3D droplet silhouette fix (`dripFrag`).** The bead+neck coverage was combined with `smin` (smooth MIN = intersection); the neck term is zero across the whole bead body (gated by `step(y, beadCenterY)`), so `smin(bead, 0)` collapsed to ~0 and every droplet hit `discard`. Added a `smax` (smooth union) helper next to `smin` and switched to `saturate(smax(bead, neck, 0.15))`.
- **Surface-relative matcaps (`BlendMatcapLayer`, `SurfaceOutputStandardLatex`, `BRDF_Latex_GGX`).** The single additive `o.Matcap` field was replaced by two carriers `MatcapMul` / `MatcapAdd`. Each of the three layers composites via `void BlendMatcapLayer(inout half3 mcMul, inout half3 mcAdd, half3 mc, float mask, int mode)`: Add → `mcAdd += mc*mask`; Replace → `mcMul *= (1-mask); mcAdd = mcAdd*(1-mask) + mc*mask`; Multiply → `half3 f = lerp(1, mc, mask); mcMul *= f; mcAdd *= f`. The BRDF applies `finalColor = finalColor * s.MatcapMul + s.MatcapAdd * matcapLitEnv` where `matcapLitEnv` is the previous `saturate(gi.diffuse + light.color*smoothstep(0,0.15,NcL)) * baseSpecOcc`. **Add** is mathematically identical to the old additive path (existing materials unchanged); **Multiply** now tints/darkens the lit surface and **Replace** overwrites it in the mask - both were broken before (Multiply on layer 1 multiplied against a zero accumulator = black; Replace == Add). The `half3 matcap` parameter was removed from `BRDF_Latex_GGX` (it reads `s.MatcapMul/MatcapAdd` directly).
- **Sampler-budget conversion.** `_BumpMap`, `_DetailNormalMap`, `_EmissionMap`, `_EmissionMap2`, `_RegionMask` moved from `sampler2D` to `UNITY_DECLARE_TEX2D_NOSAMPLER` + `UNITY_SAMPLE_TEX2D_SAMPLER(_X, _MainTex, uv)` (borrow `_MainTex`'s sampler), freeing up to 5 sampler registers toward the 16-sampler ps_5_0 cap. `_MetallicGlossMap` and `_GooMask` deliberately stay `sampler2D`/`tex2Dlod` because they are sampled in `disp` (vertex/tessellation), where a borrowed `_MainTex` sampler does not resolve.
- **VRC Light Volumes 3.x.** `Editor/cginc/LightVolumes.cginc` replaced with the upstream 3.x release. The new `LV_LightVolumeRegularSH` **excludes** additive volumes, so `surf` now calls `LV_LightVolumeRegularSH` **then** `LV_LightVolumeAdditiveSH` in the non-additive-only branch; `LV_PointLightVolumeSH`→`LV_LightVolumePointSH`. `LightVolumeEvaluate` / `LightVolumeSpecular` / `LightVolumeSpecularDominant` / `LightVolumesEnabled` names are unchanged.
- **LTCGI modes / branching / tints + LV auto-suppress (`BRDF_Latex_GGX` LTCGI block).** New props `_UseLTCGI`, `_LTCGI_Mode` (Both/Diffuse/Specular via `[ThryWideEnum]`), `_LTCGI_DiffuseColor`, `_LTCGI_SpecularColor`; new uniform `uniform float _UdonLightVolumesWithLTCGI`. The block is gated `[branch] if (_UseLTCGI > 0.5 && asint(_UdonLightVolumesWithLTCGI) == 0)`. The `asint(...) == 0` is the **auto-suppress**: the LTCGI→Light-Volumes adapter sets that global (via `Int32BitsToSingle`) to a non-zero bitmask of LTCGI-baked volumes, so when a world bakes LTCGI into the LV atlas the suit's **direct** LTCGI sampling is skipped (the light is already in the LV sample) - prevents double-bright. `_LTCGI_Mode` masks the diffuse vs specular terms (already returned separately by `LTCGI_Contribution`); the colour tints multiply each. `_UseLightVolumes` likewise gates the LV sample block. The editor now derives `LTCGI_ENABLE` / `LIGHTVOLUMES_ENABLE` from `_UseLTCGI` / `_UseLightVolumes` (was `_LTCGI_Int` / `_LV_Int > 0.001`), so VRCFury can toggle them at runtime while the keyword stays compiled.
- **Render-mode dropdown (`_Mode`).** Changed from `[Enum(Opaque,0,Cutout,1,Fade,2,Transparent,3)]` to `[ThryWideEnum(...)]` so `Thry.ShaderEditor` actually renders the dropdown - plain Unity `[Enum]` is not surfaced by ThryEditor (Poiyomi uses `[ThryWideEnum]`). The C# `SetupMaterialWithBlendMode` blend/queue/tag/keyword setup is unchanged.
- **AreaLit removed entirely.** The `AREALIT_ENABLE` shader_feature, the `_AreaLit_*` properties + uniforms, the `#include ".../AreaLit/AreaLit_Latex.cginc"`, and the `BRDF_Latex_GGX` AreaLit block (`ShadeAreaLitLatex`) are all gone; `Editor/cginc/AreaLit/` is deleted and the editor drops the `areaLit` keyword + `AREALIT_ENABLE` from `s_managedKeywords`. `baseMS` and `reflMask` (shared with the LTCGI path) are kept. Reason: AreaLit produced artifacts in worlds that have an AreaLit broadcaster present but no active light data.

### TechnicallySane Lighting: TSLCE primary + GlobalRGB fallback (2026-06-25)

Applies to **both** twins. The base shader already carried the legacy GlobalRGB reader (re-merged from the parked variant under the new licence); this pass makes **TSLCE the primary transport with GlobalRGB as fallback**, and adds the **whole** integration to the SPS twin (it previously had none).

- **Licence.** `Licenses/LICENSE-TechnicallySane.md` (TSLCE Render Texture Access License) grants permission to *read* the global control render texture `_Udon_TS_LCE` from third-party shaders. We read **only** that texture; we do not copy/redistribute any TSLCE asset. The licence warns the texture layout may change across versions, hence the liveness probe + graceful no-op below.
- **Properties (`m_techsane` "TechnicallySane V3" section).** Toggle relabelled `_UseTS` "Enable TechnicallySane Lighting" (was "...GlobalRGB"). Knobs unchanged: `_TS_Str` (Range 0-4), `_TS_Sat` (desaturate toward luma, 0-1), `_TS_Albedo` (0 = flat emissive glow, 1 = albedo-tinted bounce). Two new Thry helpbox props: `_TS_Attribution` (`[Helpbox]` + `onClick:{type:URL,...}` → techsane.gumroad.com) and `_TS_Reminder` (`[Helpbox]` + `condition_show:(_UseTS==1)` - shows only while enabled, noting it is inert/free in non-TS worlds). Helpbox props are UI-only (no uniform; same pattern as the `_h_*` props), so they need no CGPROGRAM declaration.
- **Uniforms.** `UNITY_DECLARE_TEX2D_NOSAMPLER(_Udon_TS_LCE)` added next to `_Udon_TS_GlobalRGB`; both borrow `_MainTex`'s sampler via `UNITY_SAMPLE_TEX2D_SAMPLER_LOD(..., _MainTex, ...)` = 0 of the 16 ps_5_0 sampler registers.
- **Shading (`LightingStandardLatex`, after the VRSL-GI `#endif`, before matcap).** Runtime float-gated `if (_UseTS > 0.5)` inside `#ifndef SHADER_TARGET_SURFACE_ANALYSIS` (a `GetDimensions` on a possibly-unbound global is invalid in MojoShader surface analysis). **Primary** path: `_Udon_TS_LCE.GetDimensions(w,h); if (w > 16)` (the LCE is 1024×64, so a present rig reads ~1024). The LCE is a per-fixture control texture: **column = fixtureID** at `u = (f + 0.5)/1024`, **row 1 = fixture colour** (`v = 1.5/64 = 0.0234375`), **row 15 = dimmer/intensity** (`v = 15.5/64 = 0.2421875`) - mapping lifted from the TSLCE fixture shaders (`appendResult20 = (0.0009765625*FixtureID, row*0.015625)`). We intensity-weight-average the first 6 fixtures (`f = 0..5`, all `LOD 0` so a no-mip control RT does not collapse): `accum += colourRow1 * dimRow15; weight += dimRow15; tsCol = accum / max(weight, 1e-4)`. Weighting by the dimmer means assigned-but-dark fixtures contribute nothing, so the result is the *currently lit* show colour (dark rig → ~0 → no tint). **Fallback** (only if the LCE is absent, i.e. a world still on the old GlobalRGB engine): the legacy `_Udon_TS_GlobalRGB` 5-tap `LOD 4` whole-frame average. Either way the colour is then desaturated toward luma by `_TS_Sat`, blended flat-glow↔`diffColor*baseEnergy*rawAO` by `_TS_Albedo`, and added to `finalColor` × `_TS_Str`.
- **Cost/fail-safe.** Primary = 12 taps, fallback = 5, all gated behind `_UseTS` AND a live texture; entering a world with no TechnicallySane engine costs nothing and tints nothing. Fixture count is a tunable constant (the `f < 6` loop bound). IDs are 0-based (`TSControlEngine_ID_Assigner` writes `_FixtureNumber = calculatedFixture - 1`), so columns 0..5 are real group-1 fixtures.

### Attribution helpboxes (both twins, 2026-06-25)

Each consumed third-party system now opens its section with a Thry attribution `[Helpbox]` carrying `--{onClick:{type:URL,data:...}}` (click the box to open the project): `_LV_Attribution` (VRC Light Volumes / RED_SIM), `_LTCGI_Attribution` (LTCGI / _pi_/pimaker), `_AudioLink_Attribution` (AudioLink / llealloo et al.), `_VRSL_Attribution` (VRSL / AcChosen), `_TS_Attribution` (TechnicallySane). URLs/authors mirror the `Licenses/LICENSE-*.md` files. Thry's `HelpboxDrawer` takes a single optional `float` = Unity `MessageType` (so `[Helpbox]` = Info); the two-arg Poiyomi form `[Helpbox(1,2)]` does **not** bind to it - use one arg or none here.

### VRC Light Volumes 3.x point-light refresh: `pointLightShading` arg (2026-06-28)

- **Upstream re-pull.** `Editor/cginc/LightVolumes.cginc` re-synced to the latest `red.sim.lightvolumes` 3.x revision (repo updated 2026-06-28), a verbatim copy. The 2026-06-15 shadow-sampler-reuse divergence is **not** re-applied - it was already dropped at the 2026-06-25 re-pull; this file declares the same 3 LV `SamplerState`s the shipping 2.8.0 copy had (`sampler_UdonLightVolume`, `sampler_UdonPointLightVolumeTexture`, `sampler_UdonPointLightVolumeShadowTexture`), so the LV sampler count is unchanged. `VRCLV_VERSION` stays 3.
- **API change consumed.** Upstream renamed the point-light sampler `LV_LightVolumePointSH` → **`LV_PointLightVolumeSH`** and replaced the old normal-length-driven mask (`LV_NormalMask`, where the `worldNormal` arg's *length* implicitly set shading strength) with an explicit `pointLightShading` strength argument feeding the new `LV_PointLightShading`, which is now light-source-**size** aware (`lightSinTheta` from source size widens the front-to-back ramp, so larger point/spot/area sources fade more softly). Internally `LV_PointLightContribution` → `LV_PointLightVolumeRawContribution` (returns raw per-light L0/L1 + normal/shadow attenuations and source size).
- **Shader wiring (`surf`, both twins).** The single point-light call became `LV_PointLightVolumeSH(IN.worldPos, lv_L0, lv_L1r, lv_L1g, lv_L1b, nWorldStable, 1.0)`. `nWorldStable` (the parallax-immune normal from the 2026-06-17 de-smudge) is still the normal; the new `1.0` reproduces the previous default shading strength (with `pointLightShading = 1`, `bias = 0.5` / `scale = 0.5` give the same `(NoL+1)/2` ramp as the old unit-normal mask for small/distant sources). `LV_LightVolumeRegularSH` then `LV_LightVolumeAdditiveSH`, `LightVolumeEvaluate`, and the `LightVolumeSpecular` / `LightVolumeSpecularDominant` paths keep identical signatures.
- **New upstream API present but unused.** The cginc now also exports `LightVolumeSHSpecular` / `LightVolumeAdditiveSHSpecular` (combined SH + per-light GGX/Smith/Schlick speculars with source-size roughness broadening) plus helpers (`LV_VisibilitySmithGGXCorrelatedFast`, `LV_FresnelSchlick`, `LV_SpecularBRDFDirection`, `LV_NormalizeSafe`). VixenWear keeps its existing dominant / per-channel SH-specular path; these are available for a future PBR-specular upgrade.
- **No cost delta.** No new material props, samplers, textures, keywords, or shader variants; the ps_5_0 16-sampler cap and the SPS surf compiler-OOM risk are unaffected. No visible change on existing materials/worlds (version gate + light-probe fallback identical).

### Decals: 4-layer zero-sampler overlay system (2026-06-28)

Applies to **both** twins (identical code; line numbers differ). Adds a `DECALS` section for placing logos / stickers / brand art / tattoos on the material. **It is its own top-level Thry tab `m_decals` (between SURFACE and LIGHTING)** - promoted 2026-06-29 from the original nested `m_start_decals`/`m_end_decals` foldout (which had rendered DECALS *inside* the SURFACE tab). The promotion is just the header type: `m_start_decals` -> `m_decals` (a plain `m_` header = top tab per Thry's `GetPropertyType`) and the `m_end_decals` close was deleted (a plain top header auto-pops the previous group; the following `m_lighting` performs the pop that closes `m_decals`, so an explicit end would double-pop). The 4 per-decal `s_start_decal{0..3}`/`s_end_decal{0..3}` sub-foldouts are unchanged and now nest under `m_decals`.

- **Zero-sampler by design.** The base shader already sits at the ps_5_0 16-sampler cap (11 `sampler2D` + 2 `UNITY_DECLARE_TEX2D` + 3 LV `SamplerState`). All 5 decal textures (`_DecalMask` + `_Decal0Tex`..`_Decal3Tex`) are declared `UNITY_DECLARE_TEX2D_NOSAMPLER` and read with `UNITY_SAMPLE_TEX2D_SAMPLER(_DecalNTex, _MainTex, uv)`, borrowing `_MainTex`'s sampler, so decals add **+0 samplers**. The only added cost is **+1 interpolator** (`uv2_MainTex` on `Input`, for the per-decal UV0/UV1 source) and VRAM for any assigned textures (defaults are the built-in `"white"`, so nothing until assigned).
- **Per-layer controls (`_Decal{0..3}*`).** Enable gate, decal texture, tint (`a` = layer strength), position / scale / rotation (+ rotation speed), L/R/D/U side offset, UV source (UV0/UV1), `_DecalMask` channel (R/G/B/A), color blend mode (Normal/Darken/Multiply/Lighten/Screen/Add/Subtract/Overlay), emission strength, smoothness offset (glossy sticker on matte latex or vice-versa), mirror (Flip/Left/Right) + symmetry (Symmetry/Flipped) on the source UV, and a Tiled toggle (repeat vs clamp-to-bounds). A single packed `_DecalMask` RGBA texture (`[ThryRGBAPacker]`) supplies the four per-layer masks.
- **Reuses ThryEditor's decal system.** Each layer uses the `[ThryDecalPositioning(_DecalNTex, _DecalNUV, _DecalNPos, _DecalNRot, _DecalNScale, _DecalNSideOffset)]` decorator (in-scene drag/scale/rotate gizmo). `vw_DecalUV` is a verbatim HLSL port of `ThryEditor/Editor/DecalGizmos.shader`'s `decalUV` (side-offset → rotate about decal center → remap the decal box to `[0,1]`), so the rendered decal tracks the gizmo box pixel-for-pixel. `_Time.z * rotationSpeed` drives animated rotation.
- **Shader plumbing.** Helpers `vw_DecalRemap` / `vw_DecalUV` / `vw_DecalMirror` / `vw_DecalBlend` plus the `VW_APPLY_DECAL(idx)` token-paste macro live just before `surf`. The macro composites one layer: source UV (UV0 `finalUV` or `IN.uv2_MainTex`) → mirror/symmetry → `vw_DecalUV` → bounds test (`saturate`/`frac` by Tiled) → sample → `coverage = texA * tintA * ChannelPick(_DecalMask, ch) * inBounds * enabled` → `o.Albedo = vw_DecalBlend(...)`, accumulate `decalEmis`, offset `decalSmooth`. The block runs in `surf` under `[branch] if (_DecalEnabled > 0.5)`, **after** the `UNITY_PASS_SHADOWCASTER` early-out (so decals never enter the shadow variant / the SPS surf compiler-OOM path) and after `o.Smoothness` is set; `decalEmis` is folded into `emisBase` at the existing emission build. Decals sit before the wet/drip block, so wet darkening rides over them.
- **No keyword.** `_DecalEnabled` (master) and `_Decal{i}Enabled` (per layer) are runtime float gates under `[branch]`, not shader keywords - matches the 2.8.0 Dynamic Branching direction and spends no VRChat keyword budget. No editor C# change: Thry auto-discovers the Properties block. The `_LIGHTINGMODE_REALISTIC` keyword that was floated alongside this work was intentionally dropped (no lighting-mode selector).
- **Enum dropdowns must use `[ThryWideEnum]`, not Unity's built-in `[Enum]` (fixed 2026-06-29).** `_Decal{0..3}UV` (UV Source) and `_Decal{0..3}MaskCh` (Mask Channel) were declared `[Enum(...)]` (on an `Int` property) and rendered in the Thry inspector as a **raw `0`** instead of a labelled dropdown, while the section's other dropdowns (`_DecalNBlend` / `_DecalNMirror` / `_DecalNSymmetry`) were already `[ThryWideEnum]` (also `Int`) and drew correctly. Switched the two to `[ThryWideEnum(...)]` to match. Mechanism: `Thry.ShaderEditor` draws through Unity's `MaterialEditor.ShaderProperty`, and Unity's `MaterialEnumDrawer` (`[Enum]`) is unreliable here - on a non-`Float`/`Range` property it bails to the default field (`MaterialPropertyDrawer.cs` `MaterialEnumDrawer.OnGUI`), which is what surfaced the `0`. `[ThryWideEnum]` is Thry's own drawer and is the reliable dropdown across this shader (also `_Mode`, `_LTCGI_Mode`, `_AL_Chrono_Idx`); **prefer it over `[Enum]` for any new dropdown.** Drawer-only change: the CGPROGRAM uniforms stay `float`, values/semantics are unchanged, so existing materials read identically.
- **Per-decal enable in the foldout header must have a toggle drawer (fixed 2026-06-29).** Each `s_start_decal{0..3}` header carries `reference_property:_Decal{i}Enabled`, and Thry draws that referenced property inline in the header using **its own drawer** in a ~15px box (`ShaderHeader.cs` `DrawBoxAndContent` -> `refProperty.Draw(..., isInHeader:true)`). `_Decal{i}Enabled` was `[HideInInspector] (..., Float)` with **no** drawer, so the header rendered a raw number field showing `0` (the reported bug). Fixed by adding `[ToggleUI]` -> `[HideInInspector][ToggleUI] _Decal{i}Enabled`, which renders the checkbox in the header. `[ToggleUI]` (not `[Toggle]`) so it stays keyword-free, matching the runtime-float-gate design (`surf` reads `_Decal{i}Enabled` as a float in `VW_APPLY_DECAL`); `[HideInInspector]` is kept so it only appears in the header, not as a duplicate row in the body. General rule: any Thry header `reference_property` must point at a property that has a `[Toggle]`/`[ToggleUI]` drawer, or it shows as a number.

### Realistic lighting refinement: horizon occlusion + spec clamp (2026-06-28)

`BRDF_Latex_GGX`, both twins. Two surgical, opt-in additions to the already-mature PBR model (no rewrite); defaults reproduce the prior look.

- **Horizon specular occlusion (`_Horizon_Occ`, default 1).** Normal-map detail can tilt the reflection vector below the geometric horizon, leaking reflections through the surface at grazing angles. New struct field `GeomNormalWS` (the geometric world normal, set in `surf` from `WorldNormalVector(IN, float3(0,0,1))`) feeds `horizonOcc = saturate(1 + dot(R_world, GeomNormalWS))^2`, lerped by `_Horizon_Occ`. It is folded into `baseSpecOcc`/`ccSpecOcc` right where `pomSpecOcc` already is, so **every** reflective term (indirect base/CC spec, LV spec/CC, LTCGI spec, VRSL spec) inherits it from one place. `1` = full occlusion, `0` = off.
- **Direct-specular firefly clamp (`_Spec_Clamp`, Range(0,10) default 10).** Very smooth latex under bright point lights sparkles; `baseSpecular`/`ccSpecular` are `min()`-clamped to `_Spec_Clamp` just before `finalColor`. Default 10 is effectively off (normal highlights stay below it); lower clamps harder. Per-channel clamp (a blown highlight is near-white anyway). Both props live in the existing `m_lighting` (LIGHTING) section, so no new `m_*` header / shaderdocs section.
### Poiyomi-style AO + baked shadow maps, spec-AA flicker fix, `//ifex` pass stripping (2026-07-09)

Applies to **both** twins. Three related changes in one realism/optimization pass.

- **`//ifex` geometry-pass stripping (compile-speed).** Each `target 5.0` geometry pass is now wrapped in Poiyomi/Thry `//ifex <gate>==0 && isNotAnimated(<gate>) ... //endex` so that **locked** materials with the feature off drop the whole pass from compilation (the ShaderOptimizer removes the lines before FXC sees them - stronger than `#ifdef`, needs no keyword/variant). Pure comments, so **unlocked behavior is byte-identical**. Gate per pass matches that pass's own `if (_UseX < 0.5) return;` early-out. **Twins differ:** base wraps Drip (`_UseDrip`), HUD (`_UseCyber`), Shard (`_UseVtxKinetic`), Trail (`_UseVtxKinetic`); SPS wraps Drip (`_UseDrip`) and Goo (`_UseGoo`) - the twins carry different geometry-pass sets. The `isNotAnimated()` guard (Poiyomi's own convention) keeps the pass when the toggle is marked animated, so a menu-toggled effect is never stripped out from under an animation.
- **LIGHTING DETAIL section: dedicated Occlusion Map + Baked Shadow Map (`_AOMap`, `_ShadowMask`).** New `m_lightdetail` top tab (between SURFACE and DECALS) adds two Poiyomi-style lighting-data maps. **Zero-sampler by design:** both are `UNITY_DECLARE_TEX2D_NOSAMPLER` read via `UNITY_SAMPLE_TEX2D_SAMPLER(_X, _MainTex, finalUV)`, borrowing `_MainTex`'s sampler (+0 samplers, same pattern as decals/normals). Both are runtime float-gated (`_UseAOMap`/`_UseShadowMask`, no keyword) with a channel picker (`_AOMap_Ch`/`_ShadowMask_Ch`) and strength.
  - *Occlusion Map* (default `"white"`) multiplies into `o.Occlusion` alongside the existing packed-channel AO (`pbrAO * _AO_Str`), so it inherits the established indirect-only application (`gi.diffuse`/LTCGI/VRSL/TS diffuse + `baseSpecOcc`/`ccSpecOcc`); direct light untouched. `o.Occlusion *= lerp(1, aoTex, _AOMap_Str)`.
  - *Baked Shadow Map* (default `"black"`) is a true baked contact shadow: new struct field `half BakedShadow = saturate(smTex * _ShadowMask_Str)`. In `BRDF_Latex_GGX` the **direct** sum is multiplied by `(1 - s.BakedShadow)` (applies in ForwardBase **and** ForwardAdd, so all direct lights are cut), and the **indirect** ambient-diffuse term by `(1 - s.BakedShadow * _ShadowMask_Indirect)` (base pass only, `_ShadowMask_Indirect` default 0.35). Indirect specular/reflections are intentionally left to the AO/spec-occ path.
- **Specular-AA flicker fix.** High-gloss latex (`_CC_Smoothness` 0.9 / `_Wet_Smoothness` 0.95) was aliasing because `_CC_Spec_AA` **defaulted to 0.0**, making the existing `GeometricSpecAA` (`step(0.0001, strength)`) a no-op. Two changes: (1) `_CC_Spec_AA` default `0.0 -> 1.0` so new materials get AA; (2) unconditional perceptual-roughness floor `#define VW_MIN_ROUGH 0.045` applied as `aBase = max(GeometricSpecAA(...), VW_MIN_ROUGH)` - protects already-saved materials and distance/mip aliasing regardless of the AA toggle. Only clamps effective smoothness above ~0.955 (wet at 0.95 is unaffected).
- **VRSL promoted to its own top tab (`m_stage`).** "STAGE LIGHTING & VRSL" (master `_UseVRSL`) was a `m_start_stage_grp`/`m_end_stage_grp` sub-foldout buried as the **last group inside the AUDIOLINK tab**; it is now a top-level tab (sibling to AUDIOLINK / LIGHT VOLUMES / LTCGI / TechnicallySane). Same mechanism as the Decals promotion: `m_start_stage_grp` -> plain `m_stage` (a plain `m_` = top tab per Thry `GetPropertyType`) and the `m_end_stage_grp` close was **deleted** (a plain top header auto-pops the previous group - here `m_stage` pops AUDIOLINK - and STAGE itself is closed by the Properties-block end since it is the last tab; an explicit `m_end` would double-pop). The inner `s_start_tog_UseVRSL` (gated on `_UseVRSL`) and `s_start_vrsl_gi` sub-foldouts are unchanged and now nest under `m_stage`. Inspector-only: no CGPROGRAM/keyword/logic change, `VRSL_ENABLE` and all `_VRSL_*` uniforms untouched.
- **EMISSIONS section reorganized into sub-foldouts + symmetric AudioLink.** The EMISSIONS tab was a flat dump, and the two emission layers were asymmetric in the menu: Layer 2's AL controls (`_AL_Band_Emis2`/`_AL_Emis2_Mod`) sat inline in EMISSIONS, while Layer 1's (`_AL_Band_Emission`/`_AL_Emis_Mod`/`_AL_Col_Blend`) were stranded in the AUDIOLINK tab's "Emission & Blending" (`s_start_global_emis`) foldout. Now EMISSIONS has three `s_` sub-foldouts: **Emission Layer 1** (color/map + its AL Band/Amplitude/Color-Blend, always shown), **Emission Layer 2** (`condition_show:_UseEmission2`), **Multi-Region Color Mask** (`condition_show:_UseRegionMask`); `_Emis_Exp` sits above them as "Emission Exposure (All Layers)" since it scales the summed emission. Layer 1's three AL props were **moved** out of AUDIOLINK (the now-empty `s_start_global_emis`/`s_end_global_emis` foldout was deleted; `m_start_al_global_grp` keeps its other sub-foldouts), so both layers expose Band + Amplitude inline and symmetrically. Pure Properties-block reorg: every property name/default is unchanged (only labels + position), so all CGPROGRAM reads (`amp_emis`/`_AL_Col_Blend`/`alWeight`/emission assembly) and uniforms are untouched - inspector-only, +0 everything. Note: Layer 1's AL rows are now always visible (were gated on `_UseAudioLink` inside the AL group); this matches Layer 2, which was never gated - if AL-gating is wanted it should be applied to both layers.
- **MatCap layers 4 & 5, Emission layers 3-5, brighter matcaps, AL gated (2026-07-09).** MatCap grew from 3 to **5 layers** (`m_matcap4`/`m_matcap5` tabs) and Emission from 2 to **5 layers** (`s_start_emis3`/`4`/`5` foldouts), each a verbatim copy of the existing pattern (MatCap: `_MatCapN` Texture2D on shared `sampler_linear_repeat` + `[branch] if(_UseMatCapN)` block using `BlendMatcapLayer`; Emission: `_EmissionMapN` `NOSAMPLER` on `_MainTex` sampler + `[branch] if(_UseEmissionN)` block folding into `emisBase`). **Zero new samplers** (+7 texture slots: 4 matcap + 3 emission maps, all borrowing existing samplers). All matcap **Intensity ranges widened `Range(0,2)` -> `Range(0,6)`** (layers 1-5) so matcaps can push super-bright/over-driven. Every emission layer's AudioLink rows (Band/Amplitude, + Layer 1 Color Blend) now carry `condition_show:{PROPERTY_BOOL,_UseAudioLink}`, so AL controls are hidden unless AudioLink is enabled - applied to all 5 layers for consistency (resolves the prior always-visible note). New MatCap 4/5 and Emission 3/4/5 default to their toggle **off**, so existing materials are unchanged. Symmetric across both twins (verified identical identifier counts).
- **MATCAPS collapsed to one top tab (3-level nesting) + per-matcap Freeze + labeled movement (2026-07-09).** The six matcap top tabs (`m_matcap_global`, `m_matcap1`..`5`) became a single **`m_matcaps`** top tab with `m_start_matcap_global` + `m_start_matcap1`..`5` **sections** under it, and each matcap section holds a `s_start_matcapN_tiling` ("Tiling & Movement") **subsection** - the 3-level `MATCAPS -> Matcap N -> Tiling` hierarchy (same nesting pattern as `m_audiolink` -> `m_start_al_global_grp` -> `s_start_global_surf`). The tiling/scroll vectors moved into that subsection and now use `[VectorLabel(Tile X, Tile Y, Offset X, Offset Y)]` / `[VectorLabel(Pan X, Pan Y, Spin, Speed)]` so the previously-hidden **vertical** controls are exposed: the code already applied `_MatCapN_Tiling.zw` as a static UV offset (`.w` = up/down) and `_MatCapN_Scroll.y` as vertical pan - up/down always worked, it was just unlabeled behind a "Tiling (XY)" caption.
- **Per-matcap Freeze (`_MatCapN_Freeze`, avatar-locked).** New helper `vw_MatcapFrozenUV(nWorld, meshUV, bias)` builds the matcap frame from the avatar's object-forward (`mul((float3x3)unity_ObjectToWorld, float3(0,0,1))`) instead of the live camera, so the matcap stops sliding as the viewer moves but still shifts as the avatar turns (reads as part of the material). Each matcap block blends `lerp(matcapBaseUV, vw_MatcapFrozenUV(...), _MatCapN_Freeze)` before rotation/tiling. Default `0` = prior live-camera behavior (non-destructive). The global `_MatCap_Bias` (View/Mesh) is unchanged. If a frozen matcap reads back-lit, flip the objFwd sign to `float3(0,0,-1)`.
- **Matcap Position Offset moved to its own `Offsets` subsection (per-matcap).** The `Tiling / Position` `[VectorLabel]` crammed Tile X/Y + Offset X/Y into one 4-field vector and the offset fields clipped off-screen. Split: `_MatCapN_Tiling` is now `[VectorLabel(Tile X, Tile Y)]` ("Tiling", 2-field), and a **new `float4 _MatCapN_Offset`** holds the static position in a dedicated `s_start_matcapN_offset` ("Offsets") subsection as `[VectorLabel(Offset X, Offset Y)]`. Code repointed `_MatCapN_Tiling.zw` -> `_MatCapN_Offset.xy` in all 5 blocks (Offset X = side, Offset Y = up/down). The up/down movement always worked; it was just hidden in the packed vector's unlabeled `.zw`. +5 `float4` uniforms, +0 samplers. (Data note: any prior offset stored in `_MatCapN_Tiling.zw` resets to 0 - offsets were undiscoverable before, so impact is nil.)

### Light Volumes: cginc re-sync + per-point-light specular option (2026-07-09)

- **Bundled `Editor/cginc/LightVolumes.cginc` re-synced 2026-07-04 -> 2026-07-08 source (V3).** Pure verbatim vendored copy (no local mods), so it was overwritten wholesale from `red.sim.lightvolumes/Shaders/LightVolumes.cginc`. Upstream changes: point-light attenuation-LUT `textureId` off-by-one **bugfix** (`+customId-1` -> `+customId`); internal `LV_PointLightShading`/`LV_PointLightVolumeShadowMask` simplified (dropped the light-spread term + `pointLightShadingScale` param); public `pointLightShading` default `1 -> 3`. **Our shader is unaffected by the default change** - it passes `pointLightShading` explicitly, and every called signature (`LV_LightVolumeRegularSH`, `LV_LightVolumeAdditiveSH`, `LV_PointLightVolumeSH`, `LV_PointLightVolumeSHSpecular`, `LightVolumeSpecular`/`Dominant`, `LightVolumeEvaluate`) is unchanged.
- **Per-point-light Volume specular (`_LV_Pt_Spec`) + exposed shading (`_LV_Pt_Shading`), both twins.** Learned from REDSIM's reference `Light Volume PBR.shader`: he routes point lights through `LightVolumeSHSpecular` for size-aware individual specular, whereas we derived point-light specular only from the dominant SH direction. New opt-in toggle `_LV_Pt_Spec` (default 0): when on, the point-light loop uses `LV_PointLightVolumeSHSpecular(worldPos, nWorldStable, viewDir, o.Smoothness, f0=lerp(0.04,Albedo,Metallic), _LV_Pt_Shading, ...lv_L0..lv_L1b, lvPointSpec)` and the accurate `lvPointSpec` is added to `o.LVSpec` alongside the existing SH-dominant/volume specular (`(lvSpec + lvPointSpec) * _LV_Spec_Mix * _LV_Int`); when off, it falls back to the prior `LV_PointLightVolumeSH`. The hardcoded `pointLightShading = 1.0` became the property `_LV_Pt_Shading` (Range 0-4, default 1.0 preserves the look; cginc default is 3, REDSIM's reference uses 2). Base-layer only for now (clearcoat keeps its SH specular). Our integration otherwise already matches/exceeds the reference (we already had the `_LV_Bias` normal offset, dual base+clearcoat LV specular, and `_LV_Pt_Stability`). +2 float props/uniforms, +0 samplers/keywords.
- **Parked:** REDSIM's upcoming **fog volumes** (the beautiful 2026-07-09 test renders used them) are a potential future quality win; not yet in the vendored package - revisit when shipped.

- **Cost:** AO/shadow +2 texture slots + struct field + define; matcap/emission expansion +7 texture slots + ~40 float/color/vec props + 7 `UNITY_DECLARE_TEX2D` (matcap) / `NOSAMPLER` (emission) decls; matcap restructure +5 `_Freeze` + 5 `_Offset` props/uniforms + 1 helper; LV +2 float props/uniforms. **+0 samplers, +0 keywords, +0 variants** across the whole pass (still under the ps_5_0 16-sampler cap). VRSL promotion + EMISSIONS/MATCAPS reorg are inspector-only aside from the Freeze/Offset and LV specular code.
...
### Lighting refresh shared with base + new SPS geometry passes (2026-06-25)

- **Shared with the base shader.** SPS received the same 2026-06-25 changes documented in the base-shader section above: the parallax self-shadow fix, the 3D droplet `smax` silhouette fix, surface-relative matcaps (`MatcapMul`/`MatcapAdd` + `BlendMatcapLayer`), the `sampler2D`→`UNITY_DECLARE_TEX2D_NOSAMPLER` budget conversion (same 5 fragment maps; `_MetallicGlossMap`/`_GooMask` kept), the VRC Light Volumes 3.x call-site adaptation, LTCGI modes/branching/tints + the `asint(_UdonLightVolumesWithLTCGI)` auto-suppress, the `[ThryWideEnum] _Mode` dropdown fix, and the full AreaLit removal.
- **3D water droplets ported to SPS (new pass).** The base shader's `dripVert` / `dripGeom` / `dripFrag` geometry pass was copied **verbatim** (verified byte-identical to base) into SPS as a second `ForwardBase` pass after the main surf pass. Geometry shaders are a separate stage from tessellation, so this works on SPS even though SPS can't use `tessellate:tessEdge`. It is its own compilation unit (declares `Texture2D _DripMask` + `SamplerState sampler_DripMask` locally), independent of the VRCFury-SPS-injected main vertex stage, and early-outs when `_UseDrip` / `_Drip3D_Strength` are off so cost is negligible when disabled. This brings SPS to droplet parity with the base shader.
- **Tessellation-free geometry goo-melt (new, SPS-only: `gooVert` / `gooGeom` / `gooFrag`).** The base shader melts goo via `tessellate:tessEdge` + vertex displacement in `disp`; SPS can't tessellate, so its vertex-stage goo (still present in `disp`) only moves existing verts and reads as subtle. This new geometry pass (a third `ForwardBase` pass) extrudes a billboarded melt tendril per qualifying triangle. `gooGeom` (`[maxvertexcount(4)]`) early-outs on `_UseGoo`/`_Goo_Strength`, then on `_GooMask` + an underside test `underside = lerp(1, saturate(-N.y), _Goo_Droop)`. Tendril length `= (_Goo_Strength*0.12 + _Goo_Reach*0.02) * vary * flow * underside * mask`, hangs along world-down (optionally body-following via `_Goo_BodyFollow`), sways with `_Goo_Sway` / `_Goo_SwaySpeed`, is pulled toward `_Goo_GroundY` by `_Goo_ToGround`, and pools at the floor when `_Goo_FloorCollide`. The quad is billboarded (`bRight = cross(hangDir, viewDir)`); `gooFrag` tapers it to a point (`taper = lerp(1 - y*0.8, 1, pool)`) and shades it as glossy latex (`_Color * _CC_Tint` + `unity_SpecCube0` sheen + a tight spec). It is modeled tightly on the proven droplet pass to keep the SPS surface-shader compiler within budget. Constants (length/width scales, sheen) are first-pass and need Unity visual tuning. **SPS-only** - the base shader does not have this pass (it uses tessellation instead).
